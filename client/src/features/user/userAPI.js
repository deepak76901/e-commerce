export const fetchLoggedInUserOrders = async (userId) => {
  const response = await fetch(
    `${import.meta.env.VITE_BACKEND_URL}/api/user/orders/` + userId
  );
...
export const fetchLoggedInUser = async (userId) => {
  const response = await fetch(
    `${import.meta.env.VITE_BACKEND_URL}/api/user/fetchuser/` + userId
  );
  const data = await response.json();
  return data;
};
...
export const updateUser = async (update) => {
  const response = await fetch(
    `${import.meta.env.VITE_BACKEND_URL}/api/user/updateUser/` + update.id,
    {
      method: "PATCH",
...
export const storeImageInDatabase = async ({ downloadURL, userId }) => {
  try {
    const response = await fetch(
      `${import.meta.env.VITE_BACKEND_URL}/api/user/saveImage/${userId}`,
      {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
